...
class RegisterController extends Controller
{
	...
	{
		$result = Register::where('id',$request->id)->first();
	...
	{ 
		if($request->isMethod('post')){
			...
			$requestData['user_id'] 	= $request->actionid;
			...
			if($result){
				DB::commit();
				return response()->json(['message' => 'Record Fetch Successfully','data' => $result], 200);
			}else{
				DB::rollBack();
	...
	{ 
		$requestData = $request->all();
		Register::where('id', $requestData['id'])->delete();
